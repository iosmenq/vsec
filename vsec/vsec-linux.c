 ...
 * - Uses OpenSSL and standard C libraries for Linux compatibility
 * - DEK wrapping: PBKDF2 (passphrase + exe_hash) and RSA-OAEP
 * - Optional header signing: RSA-PSS-SHA256
 *
 * Build on Linux:
 *   gcc -O2 -Wall -Wextra -o vsec vsec.c -lssl -lcrypto
 *
 * Notes:
...
#define SAFE_OPENSSL_FREE(p) do { if ((p) != NULL) { OPENSSL_free(p); (p)=NULL; } } while(0)
...
/* ---------- OpenSSL RSA key loading ---------- */

static RSA *load_rsa_public_key(const char *path) {
    ...
    if (!rsa) {
        rewind(fp);
        fseek(fp, 0, SEEK_END);
        long der_len = ftell(fp);
        rewind(fp);
        
        if (der_len > 0) {
            unsigned char *der = malloc(der_len);
            if (der) {
                if (fread(der, 1, der_len, fp) == (size_t)der_len) {
...
static RSA *load_rsa_private_key(const char *path) {
    FILE *fp = fopen(path, "r");
    if (!fp) return NULL;
...
static int sha256_of_file_path(const char *path, unsigned char out[SHA256_DIGEST_LENGTH]) {
    size_t len;
...
static int aes_ctr_crypt(const uint8_t key[32],
    ...
    int outl = 0;
    if (!EVP_EncryptUpdate(ctx, buf, &outl, in, (int)in_len)) {
        EVP_CIPHER_CTX_free(ctx);
        free(buf);
        return -1;
    }
    ...
    int final_outl = 0;
    if (!EVP_EncryptFinal_ex(ctx, buf + outl, &final_outl)) {
        EVP_CIPHER_CTX_free(ctx);
        free(buf);
        return -1;
    }
...
static void derive_keys_from_dek(const uint8_t dek[DEK_LEN], uint8_t aes_key[32], uint8_t mac_key[32]) {
    ...
    HMAC(EVP_sha256(), dek, DEK_LEN, (const unsigned char*)label_enc, strlen(label_enc), aes_key, NULL);
    HMAC(EVP_sha256(), dek, DEK_LEN, (const unsigned char*)label_mac, strlen(label_mac), mac_key, NULL);
}

/* ---------- RSA OAEP and RSA-PSS helpers ---------- */

static unsigned char *rsa_oaep_encrypt_with_key(RSA *rsa, const unsigned char *in, size_t inlen, size_t *outlen) {
    if (!rsa) return NULL;
    
    size_t rsa_size = RSA_size(rsa);
    unsigned char *out = malloc(rsa_size);
    if (!out) return NULL;
    
    int result = RSA_public_encrypt((int)inlen, in, out, rsa, RSA_PKCS1_OAEP_PADDING);
    if (result <= 0) {
        free(out);
        return NULL;
    }
...
static unsigned char *rsa_oaep_decrypt_with_key(RSA *rsa, const unsigned char *in, size_t inlen, size_t *outlen) {
    if (!rsa) return NULL;
    ...
    int result = RSA_private_decrypt((int)inlen, in, out, rsa, RSA_PKCS1_OAEP_PADDING);
    if (result <= 0) {
        free(out);
        return NULL;
    }
...
static unsigned char *rsa_pss_sign_with_key(RSA *rsa, const unsigned char *data, size_t datalen, size_t *siglen) {
    if (!rsa) return NULL;
    
    size_t rsa_size = RSA_size(rsa);
    unsigned char *sig = malloc(rsa_size);
    if (!sig) return NULL;
...
static int rsa_pss_verify_with_key(RSA *rsa, const unsigned char *data, size_t datalen, const unsigned char *sig, size_t siglen) {
    if (!rsa) return 0;
    
    unsigned char hash[SHA256_DIGEST_LENGTH];
    SHA256(data, datalen, hash);
    
    if (RSA_verify(NID_sha256, hash, SHA256_DIGEST_LENGTH, sig, (unsigned int)siglen, rsa) != 1) {
        return 0;
    }
...
static void usage(const char *prog) {
    fprintf(stderr,
        "vsec - professional .vsec encrypt/decrypt (Linux port)\n\n"
        "Usage (encrypt):\n"
...
int main(int argc, char **argv) {
    ...
    if (r <= 0 && opt.argv0 && opt.argv0[0]) {
        strncpy(exe_path, opt.argv0, sizeof(exe_path)-1);
    } else if (r > 0) {
        exe_path[r] = '\0';
    }
    
    unsigned char exe_hash[SHA256_DIGEST_LENGTH];
    ...
    if (opt.encrypt) {
        ...
        /* Generate DEK */
        uint8_t dek[DEK_LEN];
        if (RAND_bytes(dek, DEK_LEN) != 1) { fprintf(stderr, "RAND failed.\n"); SAFE_FREE(plain); return 1; }
        ...
        if (RAND_bytes(iv_file, IV_LEN) != 1) { fprintf(stderr, "RAND failed.\n"); SAFE_FREE(plain); return 1; }
        ...
        uint8_t tag[TAG_LEN];
        HMAC(EVP_sha256(), mac_key, sizeof(mac_key), hmac_input, IV_LEN + ciphertext_len, tag, NULL);
        SAFE_FREE(hmac_input);

        /* Symmetric wrap DEK with KEK from passphrase+exe_hash via PBKDF2 */
        uint8_t salt[SALT_LEN];
        if (RAND_bytes(salt, SALT_LEN) != 1) { SAFE_FREE(ciphertext); return 1; }
        ...
        uint8_t *kdf_input = malloc(passlen + SHA256_DIGEST_LENGTH);
        if (!kdf_input) { SAFE_FREE(ciphertext); return 1; }
        memcpy(kdf_input, opt.passphrase, passlen);
        memcpy(kdf_input + passlen, exe_hash, SHA256_DIGEST_LENGTH);
        uint8_t kek[32];
        if (PKCS5_PBKDF2_HMAC((const char*)kdf_input, (int)(passlen + SHA256_DIGEST_LENGTH),
                              salt, SALT_LEN, PBKDF2_ITER, EVP_sha256(), 32, kek) != 1) {
        ...
        HMAC(EVP_sha256(), kek, sizeof(kek), (const unsigned char*)"dek_enc", 7, dek_aes, NULL);
        HMAC(EVP_sha256(), kek, sizeof(kek), (const unsigned char*)"dek_mac", 7, dek_mac, NULL);

        uint8_t iv_dek[IV_LEN];
        if (RAND_bytes(iv_dek, IV_LEN) != 1) { SAFE_FREE(ciphertext); return 1; }
        ...
        HMAC(EVP_sha256(), dek_mac, sizeof(dek_mac), h_in, IV_LEN + enc_dek_sym_len, tag_dek, NULL);
        SAFE_FREE(h_in);
        ...
        RSA *rsa_pub = NULL;
        if (opt.pubkey_path) {
            rsa_pub = load_rsa_public_key(opt.pubkey_path);
            if (!rsa_pub) { fprintf(stderr, "Failed to load recipient public key.\n"); SAFE_FREE(ciphertext); SAFE_FREE(enc_dek_sym); return 1; }
            enc_dek_rsa = rsa_oaep_encrypt_with_key(rsa_pub, dek, DEK_LEN, &enc_dek_rsa_len);
            RSA_free(rsa_pub);
        ...
        char exe_hash_hex[SHA256_DIGEST_LENGTH*2 + 1];
        for (int i=0;i<SHA256_DIGEST_LENGTH;i++) sprintf(exe_hash_hex + i*2, "%02x", exe_hash[i]);
        exe_hash_hex[SHA256_DIGEST_LENGTH*2] = '\0';
        ...
        if (opt.signpriv_path) {
            RSA *signpriv = load_rsa_private_key(opt.signpriv_path);
    ...
    cleanup_encrypt:
        ...
        if (!header_is_json) {
            ...
            if (header_len >= (IV_LEN + TAG_LEN)) {
                ...
                uint8_t kek[32];
                if (PKCS5_PBKDF2_HMAC((const char*)kdf_input, (int)(passlen + SHA256_DIGEST_LENGTH),
                                      iv_file, IV_LEN, PBKDF2_ITER, EVP_sha256(), 32, kek) != 1) {
                ...
                HMAC(EVP_sha256(), mac_key, sizeof(mac_key), h_in, IV_LEN + ciph_len, calc_tag, NULL);
                SAFE_FREE(h_in);
        ...
        if (sig_b64 && opt.verifypub_path) {
            ...
            RSA *verpub = load_rsa_public_key(opt.verifypub_path);
        ...
        /* Extract base64 fields from header_obj */
        char *tmp;
        ...
        unsigned char local_exe_hash[SHA256_DIGEST_LENGTH];
        memset(local_exe_hash, 0, sizeof(local_exe_hash));
        if (exe_path[0]) {
            if (sha256_of_file_path(exe_path, local_exe_hash) == 0) {
                char local_hex[SHA256_DIGEST_LENGTH*2 + 1];
                for (int i=0;i<SHA256_DIGEST_LENGTH;i++) sprintf(local_hex + i*2, "%02x", local_exe_hash[i]);
                local_hex[SHA256_DIGEST_LENGTH*2] = '\0';
        ...
        if (enc_dek_sym_b64 && salt_b64 && iv_dek_b64 && tag_dek_b64) {
            if (!opt.passphrase) {
                ...
                uint8_t *kdf_input = malloc(passlen + SHA256_DIGEST_LENGTH);
                memcpy(kdf_input, opt.passphrase, passlen);
                memcpy(kdf_input + passlen, local_exe_hash, SHA256_DIGEST_LENGTH);
                uint8_t kek[32];
                if (PKCS5_PBKDF2_HMAC((const char*)kdf_input, (int)(passlen + SHA256_DIGEST_LENGTH),
                                      salt, (int)salt_len, PBKDF2_ITER, EVP_sha256(), 32, kek) != 1) {
                    ...
                    HMAC(EVP_sha256(), dek_mac, sizeof(dek_mac), tmp_in, iv_dek_len + enc_dek_sym_len, calc_tag_dek, NULL);
                    SAFE_FREE(tmp_in);
    ...
    try_rsa:
        if (!got_dek && enc_dek_rsa_b64 && opt.privkey_path) {
            ...
            RSA *priv = load_rsa_private_key(opt.privkey_path);
        ...
        uint8_t calc_tag[TAG_LEN];
        HMAC(EVP_sha256(), mac_key, sizeof(mac_key), h_in2, iv_file_len + ciph_len, calc_tag, NULL);
        SAFE_FREE(h_in2);
        if (memcmp(calc_tag, tag_file, TAG_LEN) != 0) {
